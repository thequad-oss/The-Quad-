...
self.addEventListener('push', e => {
  ...
  // Try JSON payload (works when push is encrypted)
  try {
  ...
  // Fallback: FCM strips unencrypted body so e.data is null.
  // Edge function appends #like/#comment/#reply to the URL
  // so we can still read the type without encryption.
  if (!type) {
    try {
      const raw2 = e.data?.text();
      const data2 = JSON.parse(raw2 || '{}');
      const hash = (data2.url || '').split('#')[1] || '';
      if (hash && LABELS[hash]) { type = hash; url = data2.url.split('#')[0]; }
    } catch (_) {}
  }
